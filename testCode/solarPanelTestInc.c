...
void solarPanelControl(void* SPD) {
  ...
  static int solarPanelProgress = -100000; //progress to fully retracted or deployed
  ...
  if(*(data->timer) % 10 == 0) {
    ...
    *(data->driveMotorSpeedInc) = TRUE;
    ...
    int dutyCycleNano = 250 - ((motorDrive * 250) / 100); //convert % to nanoseconds
    FILE *duty;
